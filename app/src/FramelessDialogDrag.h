...
public:
    ...
    bool move(QDialog *dialog, const QMouseEvent *event)
    {
        if (!m_active)
            return false;
        QPoint target = event->globalPosition().toPoint() - m_grab;
        const QScreen *screen = dialog->screen()
            ? dialog->screen() : QGuiApplication::primaryScreen();
        if (screen) {
            const QRect avail = screen->availableGeometry();
            const QSize size = dialog->frameGeometry().size();
            // Fully on screen: clamp the top-left so the whole frame fits.
