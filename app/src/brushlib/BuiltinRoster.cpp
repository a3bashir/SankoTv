...
// ---------------------------------------------------------------------------
// The ERASER roster. DELIBERATELY SEPARATE from builtinRoster(): the
// combined preview SHA (193847fa...) fingerprints exactly builtinRoster()'s
// renders, and the >= 10-per-category checks iterate builtinCategories() -
// erasers appearing in either would move a pinned baseline for a reason
// that has nothing to do with brush rendering. This roster has its OWN
// pinned SHA and its own count in the library test.
//
// Eight, not ten: ten was the BRUSH categories' bar, inherited from a check
// written for them, not a design number. Each eraser here has a distinct
// mechanical identity in the parameters that matter on the mask path -
// hardness, tips, pressure curves, spacing, grain-on-coverage. All are
// eraseMode by construction, and eraseMode itself forces the mask path
// (smudgeActive / dualBrushEnabled / usesColorStrokeBuffer all return
// false under it), so a contradictory eraser cannot even be built.
QVector<BrushPreset> builtinEraserRoster()
{
    QVector<BrushPreset> r;
    const QString kEraser = QStringLiteral("Eraser");
    auto makeEraser = [&](const QString &name,
                          const std::function<void(::Brush &)> &recipe) {
        BrushPreset p = make(kEraser, name, recipe);
        p.id = QStringLiteral("eraser/%1").arg(slug(name));
        p.brush.setEraseMode(true);
        return p;
    };

    r << makeEraser(QStringLiteral("Hard Round"), [&](::Brush &b) {
        // The classic eraser, and the default: hard edge, gentle
        // opacity-pressure so a light pen lifts less than a firm one.
        b.setSize(40); b.setHardness(1.0); b.setOpacity(1.0);
        b.setSpacing(0.08);
        b.opacityPressureCurve().setControlPoints(curve2(0.35, 1.0));
    });
    r << makeEraser(QStringLiteral("Soft Round"), [&](::Brush &b) {
        b.setSize(60); b.setHardness(0.25); b.setOpacity(1.0);
        b.setSpacing(0.08);
        b.opacityPressureCurve().setControlPoints(curve2(0.3, 1.0));
    });
    r << makeEraser(QStringLiteral("Pressure Eraser"), [&](::Brush &b) {
        // Steep size AND opacity response: light touch lifts, full
        // pressure cuts through.
        b.setSize(50); b.setHardness(0.7); b.setOpacity(1.0);
        b.setSpacing(0.07);
        b.sizePressureCurve().setControlPoints(curve2(0.15, 1.0));
        b.opacityPressureCurve().setControlPoints(curve2(0.1, 1.0));
    });
    r << makeEraser(QStringLiteral("Airbrush Eraser"), [&](::Brush &b) {
        b.setSize(90); b.setHardness(0.12); b.setOpacity(0.55);
        b.setFlow(0.25); b.setSpacing(0.05);
        b.opacityPressureCurve().setControlPoints(curve2(0.15, 1.0));
    });
    r << makeEraser(QStringLiteral("Pencil Eraser"), [&](::Brush &b) {
        b.setSize(12); b.setHardness(0.6); b.setOpacity(0.85);
        b.setSpacing(0.1); b.setGrainDepth(0.3);
        b.sizePressureCurve().setControlPoints(curve2(0.4, 1.0));
        b.opacityPressureCurve().setControlPoints(curve2(0.3, 1.0));
    });
    r << makeEraser(QStringLiteral("Chalk Eraser"), [&](::Brush &b) {
        b.setSize(70); b.setHardness(0.55); b.setOpacity(0.9);
        b.setSpacing(0.12); b.setGrainDepth(0.7);
        b.opacityPressureCurve().setControlPoints(curve2(0.25, 1.0));
    });
    r << makeEraser(QStringLiteral("Chisel Eraser"), [&](::Brush &b) {
        b.setSize(55); b.setHardness(0.95); b.setOpacity(1.0);
        b.setSpacing(0.09);
        b.setTipRoundness(0.28); b.setTipAngle(38.0);
        b.opacityPressureCurve().setControlPoints(curve2(0.35, 1.0));
    });
    r << makeEraser(QStringLiteral("Scatter Eraser"), [&](::Brush &b) {
        b.setSize(45); b.setHardness(0.8); b.setOpacity(0.9);
        b.setSpacing(0.35);
        b.setScatterAlong(0.8); b.setScatterPerpendicular(0.8);
        b.setScatterCount(3); b.setSizeJitter(0.4);
        b.opacityPressureCurve().setControlPoints(curve2(0.3, 1.0));
    });
